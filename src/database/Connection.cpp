...
Connection::Connection()
{
    try
    {
        conn = new connection("dbname=mlpdb host=localhost user=mlpuser password=mlpuser");
    }
    ...
    try
    {
        ...
        conn->prepare("insertAttribute",
            ...
            ("INTEGER")("INTEGER")("VARCHAR", prepare::treat_string)("SMALLINT")("INTEGER");
        ...
        conn->prepare("insertInstance",
            ...
            ("INTEGER")("INTEGER")("INTEGER")("NUMERIC")("VARCHAR", prepare::treat_string);

        // Seleção do último ID de Relation gerado
        conn->prepare("selectLastID", "SELECT currval(pg_get_serial_sequence('Relation', 'relationid'))");
    }
    catch(pqxx_exception &e)
    {
